...
export const getCommentsByVideo = async (req: Request, res: Response) => {
  try {
    ...
    const comments = await Comment.find({ video: videoId, parentComment: null }).populate('user', 'username');
...
export const getRepliesByComment = async (req: Request, res: Response) => {
  try {
    ...
    const replies = await Comment.find({ parentComment: commentId }).populate('user', 'username');
